// Client-side mock auth for the DRISHTIQ preview.
// Persists a fake session in localStorage — no backend, no real security.
...
export function signIn(email: string, workspace?: string): MockSession {
  const name = email
    .split("@")[0]
    .split(/[._-]/)
    .filter(Boolean)
  ...
  const session: MockSession = {
    user: {
      ...
      initials,
      workspace: workspace ?? DEFAULT_WORKSPACES[0],
    },
